    ...
    docs: [
      {
        ...
        data: () => ({
          ...
          des*/
beforeEach(() => { //vebtns student before
  getDocs.mockResolvedValue({
    docs: [
...
// just a helper to not repeat wrapper stuff
const renderForm = () =>
  render(
    <MemoryRouter>
      <OnboardingForm />
...
describe("OnboardingForm", () => {
  ...
  test("shows trade interest select", () => {
    renderForm();
    // basic check for trade dropdown
  ...
  test("work experience section is there, both instances", () => {
    renderForm();
    expect(screen.getByRole("heading", { name: "Add Work Experience" })).toBeInTheDocument();
  ...
  test("skill tags thing shows", async () => {
    renderForm();
    // not instant so need wait
    expect(await screen.findByLabelText("Skills")).toBeInTheDocument();
  });
  test("wraps with finish setup button", () => {
    renderForm();
    // last action btn
